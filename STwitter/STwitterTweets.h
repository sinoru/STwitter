...
+ (id)statusUpdate:(NSString *)status account:(ACAccount *)account inReplyToStatusID:(NSNumber *)inReplyToStatusID latitude:(NSNumber *)latitude longitude:(NSNumber *)longitude placeID:(NSNumber *)placeID displayCoordinates:(BOOL)displayCoordinates error:(out NSError **)error __OSX_AVAILABLE_STARTING(__MAC_NA,__IPHONE_5_0);
